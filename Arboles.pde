...
boolean searching = false, firstFound = false;
...
void searchingVertex() {
 ...
 if (searching == true) {
   ...
   textSize(10);
   if (keyCode == ENTER) {
     println(searchVertex.get(0).vertexName);
     findPath(myVertex.get(0));
     searching = false; 
   }
...
void findPath(Vertex vertex) {
  if (vertex.vertexName == searchVertex.get(0).vertexName) {
    println("FELICIDADES, LO ENCONTRASTE");
  }
  else {
    for (int i = 0; i < myEdges.size(); i++) {
      if (myEdges.get(i).vertexRoot.equals(vertex)) {
        searchPathTo.add(myEdges.get(i).vertexRoot);
        findPath(myEdges.get(i).vertexChild);
      } else {
        searchPathTo.clear();
      }
    }
